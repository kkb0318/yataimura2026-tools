...
- A4に収まる数を自動計算して配置
- 件数が多い場合は自動で複数ページ化
- 宝探しテーマ（古地図カラー、カード背景、中央宝箱ロゴ）で出力
...
- 各QRは古地図風カード（ベージュ背景 + 茶色ボーダー）に配置されます。
- QR本体は濃い焦げ茶色で描画され、誤り訂正レベルは `H`（高）です。
- QR中央に小さな宝箱ロゴ（QR一辺の12%）を描画します。
- `--caption index` のとき、QR下に巻物タグ風のラベル帯を表示します。
- `label` が長い場合は **12文字で固定切り詰め**（`13文字目以降は…`）です。
